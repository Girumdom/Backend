...
async function createUser(email, password_hash, username, user_type) { // Function to create a new user
    if (!email || !password_hash || !username || !user_type) {
        throw new Error('Email, password_hash, username, and user_type are required to create a user');
    }
    try {
        const [result] = await pool.query(
            'INSERT INTO USER (email, password_hash, username, user_type) VALUES (?, ?, ?, ?)',
            [email, password_hash, username, user_type]
        );
...
async function updateUser(user_id, email, password_hash, username) { // Function to update an existing user
    if (!user_id) {
    ...
    try {
        const [result] = await pool.query(
            'UPDATE USER SET email = ?, password_hash = ?, username = ? WHERE user_id = ?',
